// CAN\NVAS.js plugin
    ...
    var i = 0;
    document.getElementById('add_element').onclick = function() {
        ...
        div3.setAttribute("class", "fa fa-user-times");
        var delete_img = document.createElement("img");
        delete_img.src = "delete.png";
        div.setAttribute("class", "row");
        div.setAttribute("style", "margin-bottom:10px;");

        var add_box = document.getElementById('meme_box');
        x.setAttribute("class", "add_canvas");
        x.setAttribute("id", i);
        ...
        // draw the image
        ctx.drawImage(image, 0, 0, x.width, x.height);

        // styles
        ctx.fillStyle = '#fff';
        ctx.strokeStyle = '#000';
        ctx.lineWidth = x.width * 0.004;

        var _textSizeTop = textSizeTop / 100 * x.width;
        var _textSizeBottom = textSizeBottom / 100 * x.width;

        // draw top text
        add_style(ctx, 'Impact', _textSizeTop, 'center', 'bottom');
        ctx.drawBreakingText(top, x.width / 2, _textSizeTop + padding, null, 1, 'fill');
        ctx.drawBreakingText(top, x.width / 2, _textSizeTop + padding, null, 1, 'stroke');

        // draw bottom text
        add_style(ctx, 'Impact', _textSizeBottom, 'center', 'top');
        var height = ctx.drawBreakingText(bottom, 0, 0, null, 1, 'none').textHeight;
        // console.log(ctx.drawBreakingText(bottom, 0, 0, null, 1, 'none'));
        ctx.drawBreakingText(bottom, x.width / 2, x.height - padding - height, null, 1, 'fill');
        ctx.drawBreakingText(bottom, x.width / 2, x.height - padding - height, null, 1, 'stroke');
    ...
    function add_style(ctx, font, size, align, base) {
        ctx.font = size + 'px ' + font;
        ctx.textAlign = align;
        ctx.textBaseline = base;
    }
